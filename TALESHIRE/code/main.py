...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption('TALESHIRE : The Unknown Legend')
        self.clock = pygame.time.Clock()
        self.encounter_timer = Timer(2000, func = self.monster_encounter)

        # player monsters 
        self.player_monsters = {
            0: Monster('Ivieron', 100),
            1: Monster('Atrox', 100),
            2: Monster('Cindrill', 100),
            3: Monster('Atrox', 100),
            4: Monster('Sparchu', 100),
            5: Monster('Gulfin', 100),
            6: Monster('Jacana', 100),
        }
        ...
        # Inventory animation
        self.inventory_alpha = 0
        self.inventory_fade_speed = 15  # semakin besar semakin cepat

        # Inventory item images
        self.inventory_images = {
            "Potion": pygame.image.load(join('..', 'graphics', 'ui', 'potion.png')).convert_alpha(),
            "Revive": pygame.image.load(join('..', 'graphics', 'ui', 'revive.png')).convert_alpha(),
            "Evo Stone": pygame.image.load(join('..', 'graphics', 'ui', 'evostone.png')).convert_alpha()
        }
        ...
        for obj in tmx_map.get_layer_by_name('Entities'):
            if obj.name == 'Player':
                ...
                Character(
                    ...
                    collision_sprites = self.collision_sprites,
                    radius = obj.properties.get('radius', 32),
        ...
        if not self.dialog_tree and not self.battle:
            ...
            if keys[pygame.K_i]:
                ...
                if self.inventory_open:
                    self.inventory_alpha = 0  # Reset animasi saat buka
        ...
        if self.inventory[item] > 0:
            if item == "Potion":
                if monster.health > 0 and monster.health < monster.get_stat('max_health'):
                    ...
                    monster.health = min(monster.get_stat('max_health'), monster.health + 500)
                ...
                # Cek apakah monster bisa evolusi
                if monster.evolution:
                    evo_name = monster.evolution[0]
                    monster.level = max(monster.level, monster.evolution[1])  # Pastikan level cukup
                    # Trigger animasi evolusi!
                    ...
                    self.player_monsters[monster_keys[self.inventory_monster_selected]] = Monster(evo_name, monster.level)
                    self.inventory_message = f"{monster.name} evolved to {evo_name}!"
                    self.inventory[item] -= 1
                else:
                    self.inventory_message = f"{monster.name} cannot evolve!"
        else:
    ...
    def show_inventory(self):
        ...
        # Fade-in animation
        if self.inventory_open and self.inventory_alpha < 200:
            self.inventory_alpha += self.inventory_fade_speed
        elif not self.inventory_open and self.inventory_alpha > 0:
            self.inventory_alpha -= self.inventory_fade_speed

        # Lebarkan surface inventory
        inv_surf = pygame.Surface((600, 400), pygame.SRCALPHA)
        inv_surf.fill((30, 30, 30, min(self.inventory_alpha, 200)))
        
        # Draw item list with images
        for idx, (item, count) in enumerate(self.inventory.items()):
            ...
            inv_surf.blit(text_surf, (170, 35 + idx * 60))  # teks lebih ke kanan
            # Draw item image
            if item in self.inventory_images:
                img = pygame.transform.scale(self.inventory_images[item], (48, 48))
                inv_surf.blit(img, (80, 25 + idx * 60))  # gambar lebih ke kanan dan besar
        ...
        for idx, monster in self.player_monsters.items():
            ...
            inv_surf.blit(text_surf, (80, y_offset + idx * 25))

        # Draw message
        if self.inventory_message and self.inventory_message_timer > 0:
            msg_surf = font.render(self.inventory_message, True, (0,255,0))
            inv_surf.blit(msg_surf, (80, y_offset + 180))
            self.inventory_message_timer -= 1
        elif self.inventory_message_timer <= 0:
            self.inventory_message = ""

        # Blit inventory surface to main display
        inv_rect = inv_surf.get_rect(center=(WINDOW_WIDTH//2, WINDOW_HEIGHT//2))
